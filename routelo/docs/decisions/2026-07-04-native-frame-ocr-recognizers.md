# Decision: recognize receipt OCR directly from native camera frame buffers

Date: 2026-07-04

## Status

Accepted for the live OCR path.

## Context

VisionCamera can now show a hidden preview probe and emit frame metadata in the
scanner. The remaining decision is how to turn live camera frames into receipt
OCR evidence without lowering accuracy or inventing data.
...
Use platform-native recognizers that process camera frame buffers directly:

- Android: native PP-OCR frame recognizer.
- iOS: Apple Vision OCR recognizer.
- Cloud fallback: CLOVA OCR only when the user explicitly consents.
...
save behavior shared across Android, iOS, and still-photo OCR.
...
- Native recognizers must fail closed and keep still-photo OCR available.
...
3. Add iOS Apple Vision binding against VisionCamera frame buffers.
4. Add device benchmark logging for real receipt samples.
...
- Apple Vision OCR produces materially worse field recall than still-photo OCR.
